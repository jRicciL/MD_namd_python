...
El comando `autoionize` toma como entrada los archivos `_temp.psf` y `_temp.pdb` que creamos en el paso anterior, y la bandera `-neutralize` se encarga de añadir iones de Cloro o Sodio según la carga del sistema. **¿Qué iones y cuántos se agregaron al sistema?** Finalmente, dos nuevos archivos `tc5b_wb.psf` y `tc5b_wb.pdb` son creados en el directorio.  
...
Para obtener las medidas de la caja de agua 
